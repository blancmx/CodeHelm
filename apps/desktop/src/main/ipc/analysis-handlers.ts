...
export function registerAnalysisHandlers(db: DatabaseInstance, tasks: AnalysisTasks = getAnalysisTasks(db)) {
  ...
  ipcMain.handle(IpcChannels.ANALYSIS_GET_TASK, async (_event, projectId: string) => tasks.get(projectId));
  ipcMain.handle(IpcChannels.ANALYSIS_START, (event, projectId: string) => {
  ...
  ipcMain.handle(IpcChannels.ANALYSIS_CANCEL, (_event, taskId: string) => tasks.cancel(taskId));
}

...
```bash
# 日常本地门禁：lint、生产构建、类型检查和 448 项单元/集成测试
pnpm check
...
升级前请保留 `%APPDATA%\CodeHelm\` 及其备份目录。最终 `main` 候选已完成本机安装、同版本重装、卸载和用户数据保留验收；v0.1 是首次发行，因此真实 `v0.1 → v0.2` 升级、失败回滚和数据保留被列为 v0.2 发布前强制门禁。不能据此承诺任意未来版本升级后数据必然保留，升级前仍应复制经验证备份。

当前主窗口和隔离确认窗口均启用 sandbox，生产 Renderer CSP 不允许 `unsafe-eval`。本地 `check:ci`、实际 Electron E2E 与 GitHub Windows CI 已通过；main 分支已要求 GitHub Actions 的 `windows-quality-gates` 检查通过且规则对管理员生效。v0.1 仍为未签名开发/内测渠道；最终源码包已通过本机安装与启动验收，但未完成干净 Windows VM 全业务流程，用户已明确接受该风险并延期到 v0.2。Windows SmartScreen 或杀毒软件仍可能显示“未知发布者”提示。`CODEHELM_USER_DATA_DIR` 仅供隔离验收/诊断，不是普通用户迁移数据的接口。
